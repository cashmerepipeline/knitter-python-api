...
from project_pb2 import ChangeProjectStatusRequest, ProjectStatus
from knitter_client import get_knitter_client_stub, login
from knitter_services.project import change_project_status
...
async def main():
    ...
    status = ProjectStatus.ProjectCanceled
    request = ChangeProjectStatusRequest(
        project_id = "0",
        status= status 
    )

    ok, m_response, details = await change_project_status(
        request, client_stub, metadata=metadata
    )

    # 打印管理表
    if ok == grpc.StatusCode.OK:
        print(m_response.status)
        assert m_response.status == status
    else:
