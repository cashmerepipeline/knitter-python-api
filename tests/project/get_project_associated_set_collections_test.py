...
from project_pb2 import GetProjectAssociatedSetCollectionsRequest
from knitter_client import get_knitter_client_stub, login
from knitter_services.project import get_project_associated_set_collections
...
async def main():
    ...
    request = GetProjectAssociatedSetCollectionsRequest(
        project_id = "0",
        collection_ids = ["0", "1"]
    )

    ok, m_response, details = await get_project_associated_set_collections(
        request, client_stub, metadata=metadata
    )

    # 打印管理表
    if ok == grpc.StatusCode.OK:
        for collection in m_response.set_collections:
