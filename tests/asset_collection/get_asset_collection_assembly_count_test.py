...
from asset_collection_pb2 import GetAssetCollectionAssemblyTotalCountRequest
from knitter_services.asset_collection import get_asset_collection_assembly_total_count
...
async def main():
    ...
    request = GetAssetCollectionAssemblyTotalCountRequest(
            collection_id = "1",
    )

    ok, m_response, details = await get_asset_collection_assembly_total_count(
        request, client_stub, metadata=metadata
    )

    # 打印管理表
    if ok == grpc.StatusCode.OK:
      print(m_response.total_count)
    else:
