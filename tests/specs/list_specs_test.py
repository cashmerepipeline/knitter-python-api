...
from knitter_services.specs import list_specs, new_specs
from specs_pb2 import ListSpecsRequest
...
async def main():
    ...
    request = ListSpecsRequest(
        owner_manage_id= ASSETS_MANAGE_ID,
        owner_entity_id="0",
    )

    ok, m_response, details = await list_specs(
        request, client_stub, metadata=metadata
    )

    # 打印管理表
    if ok == grpc.StatusCode.OK:
        for specs in m_response.specses:
            print(bson.decode(specs))
    else:
