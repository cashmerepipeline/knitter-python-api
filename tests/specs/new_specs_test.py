...
from knitter_services.specs import new_specs
from specs_pb2 import NewSpecsRequest
...
async def main():
    ...
    name = Name(language="zh", name="测试规格")
    request = NewSpecsRequest(
        name=name,
        owner_manage_id= ASSETS_MANAGE_ID,
        owner_entity_id="0",
        description="test new specs"
    )

    ok, m_response, details = await new_specs(
