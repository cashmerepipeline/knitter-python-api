...
from id_codes.manage_ids import ASSETS_MANAGE_ID, MANAGES_MANAGE_ID
from knitter_client import get_knitter_client_stub, login
from knitter_services.specs import list_specs_prefabs, new_specs
from specs_pb2 import ListSpecsPrefabsRequest
...
async def main():
    ...
    name = Name(language="zh", name="测试景")
    request = ListSpecsPrefabsRequest(
        specs_id="0",
    )

    ok, m_response, details = await list_specs_prefabs(
        request, client_stub, metadata=metadata
    )

    # 打印管理表
    if ok == grpc.StatusCode.OK:
        for prefab in m_response.prefabs:
            print(bson.decode(prefab))
    else:
