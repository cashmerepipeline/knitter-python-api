...
from prefab_pb2 import NewPrefabRequest
...
async def main():
    ...
    name = Name(language="zh", name="测试预制件")
    request = NewPrefabRequest(
        name=name,
        specs_id="0",
        description="test new prefab"
    )

    ok, m_response, details = await new_prefab(
